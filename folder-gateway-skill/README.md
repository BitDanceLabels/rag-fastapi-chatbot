...
## Hướng dẫn đăng ký gateway (service trong repo chính)
Áp dụng cho FastAPI chính (`app/main.py`) đã cắm `register_gateway`:
- Biến môi trường tối thiểu:
  - `GATEWAY_URL`: URL gateway, ví dụ `http://127.0.0.1:30090`.
  - `SERVICE_BASE_URL`: URL service mà gateway gọi tới, ví dụ `http://127.0.0.1:8000`.
  - `SERVICE_NAME`: tên hiển thị trên gateway (tùy chọn, mặc định `rag-fastapi-chatbot`).
  - `GATEWAY_PREFIX`: prefix để tránh trùng path (tùy chọn), ví dụ `/rag-bot`.
- Khởi động service: `uvicorn app.main:app --host 0.0.0.0 --port 8000` (hoặc docker compose).
- Kiểm tra log có dòng `Gateway registered (...) routes=N`. Nếu lỗi đăng ký, nó sẽ retry `REGISTER_RETRIES` lần (mặc định 5) với delay `REGISTER_DELAY` giây.
- Mở Swagger gateway và gọi thử một route qua gateway để chắc không 404/401.
- Đăng ký thủ công (nếu không dùng auto) bằng POST:
  ```bash
  curl -X POST "$GATEWAY_URL/gateway/register" \
    -H "Content-Type: application/json" \
    -d @payload.json
  ```
  trong đó `payload.json` có dạng:
  ```json
  {
    "name": "rag-fastapi-chatbot",
    "base_url": "http://127.0.0.1:8000",
    "routes": [
      {"name": "rag-search", "method": "POST", "gateway_path": "/v1/rag/search", "upstream_path": "/v1/rag/search", "summary": "RAG search", "description": "Search RAG pipeline"}
    ]
  }
  ```
